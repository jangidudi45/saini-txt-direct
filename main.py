...
keyboard = InlineKeyboardMarkup(
    [
        [InlineKeyboardButton(text="🛠️ Help", url="https://t.me/itsxadiii"), InlineKeyboardButton(text="Channel", url="https://t.me/Medicoarmy")],
    ]
)
...
async def restart_handler(_, m):
    if m.chat.id not in AUTH_USERS:
        ...
        await m.reply_text("𝗕𝗼𝘁 𝗶𝘀 𝗥𝗲𝘀𝘁𝗮𝗿𝘁𝗶𝗻𝗴...", True)
...
async def start(bot, m: Message):
    ...
    if m.chat.id in AUTH_USERS:
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton(text="📞 Contact", url=f"tg://openmessage?user_id={OWNER}"), InlineKeyboardButton(text="𝗖𝗵𝗮𝗻𝗻𝗲𝗹", url="https://t.me/Medicoarmy")],
        ])
...
async def feature_button(client, callback_query):
  ...
  keyboard = InlineKeyboardMarkup([
      ...
      [InlineKeyboardButton("💧 Watermark", callback_data="watermark_command"), InlineKeyboardButton("🔄 Restart", callback_data="Restart_command")],
...
async def restart_button(client, callback_query):
  keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Feature", callback_data="feat_command")]])
  caption = f"**🔄 Restart Command:**\n\nIf You Want to Restart Your Bot, Simply Use Command /Restart."
  await callback_query.message.edit_media(
    InputMediaPhoto(
      media="https://tinypic.host/images/2025/07/14/file_000000002d44622f856a002a219cf27aconversation_id68747543-56d8-800e-ae47-bb6438a09851message_id8e8cbfb5-ea6c-4f59-974a-43bdf87130c0.png",
      caption=caption
...
async def txt_handler(bot: Client, m: Message):  
    ...
    # Prompt for Topic ID
    await editable.edit("__**⚠️Provide the Topic ID (Thread ID) or send /d to skip__\n\n<blockquote><i>🔹 Send /id in the topic to get the Topic ID.\n🔸 Example: Topic ID = 123456\n🔹 Leave as /d if no topic or for general chat.</i></blockquote>\n**")
    try:
    ...
    if "/d" in raw_text8:
        topic_id = None  # No topic ID, send to general chat
    else:
        try:
            topic_id = int(raw_text8)  # Convert to integer for message_thread_id
        except ValueError:
            await editable.edit("**🔹Invalid Topic ID. Exiting Task...**")
            processing_request = False
            return
    ...
    try:
        if raw_text == "1":
            ...
            if "/d" not in raw_text7:
                ...
                pinning_message_id = message_id + 1
                await bot.delete_messages(channel_id, pinning_message_id, message_thread_id=topic_id)
        else:
            if "/d" not in raw_text7:
                await bot.send_message(
                    chat_id=m.chat.id,
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                url = f"https://cpapi-rjbs-1l0p.onrender.com/extract_keys?url={url}@bots_updatee&user_id={m.chat.id}"
                ...
                signed_api = f"https://cpapi-rjbs-1l0p.onrender.com/extract_keys?url={url}@bots_updatee&user_id={m.chat.id}"
    ...
    if raw_text7 == "/d":
        ...
        await bot.send_message(
            chat_id=m.chat.id,
            text=f"<blockquote><b>✅ Your Task is completed, please check your Set Channel📱</b></blockquote>"
        )
